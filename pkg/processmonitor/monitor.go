...
func (pm *ProcessMonitor) createActivityCheckers() []ActivityChecker {
	return []ActivityChecker{
		NewCPUActivityChecker(pm.logger),    // 传入 logger
		NewDiskIOActivityChecker(pm.logger), // 传入 logger
		NewMemoryActivityChecker(pm.logger), // 传入 logger
	}
}

// NewCPUActivityChecker 创建一个实际的 CPU 活动检查器。
func NewCPUActivityChecker(logger *zap.Logger) ActivityChecker {
	return &cpuActivityChecker{logger: logger}
}
...
func (c *cpuActivityChecker) CheckActivity(mp *MonitoredProcess) bool {
	p, err := mp.Process() // 获取 gopsutil 的 Process 对象
	if err != nil {
		c.logger.Error("获取进程对象失败", zap.Error(err), zap.Int("pid", mp.PID))
		return false
	}
	// 获取进程的 CPU 使用率
	// 注意：CPUPercentWithContext 第一次调用会返回 0，需要至少两次调用才能计算出实际百分比。
	// 这里简化处理，如果非 0 则认为有活动。
	percent, err := p.CPUPercentWithContext(context.Background()) 
	if err != nil {
		c.logger.Error("获取进程 CPU 使用率失败", zap.Error(err), zap.Int("pid", mp.PID))
		return false
	}
	// 如果 CPU 使用率大于某个阈值，则认为活跃
	// 阈值可以根据实际情况调整，例如 0.1% 或 1%
	return percent > 0.1
}

func (c *cpuActivityChecker) GetName() string {
	return "CPUActivityChecker"
}

// NewDiskIOActivityChecker 创建一个实际的磁盘 I/O 活动检查器。
func NewDiskIOActivityChecker(logger *zap.Logger) ActivityChecker {
	return &diskIOActivityChecker{logger: logger}
}

type diskIOActivityChecker struct {
	logger *zap.Logger
	lastIOCounters *process.IOCountersStat // 记录上一次的 I/O 计数
}

func (d *diskIOActivityChecker) CheckActivity(mp *MonitoredProcess) bool {
	p, err := mp.Process()
	if err != nil {
		d.logger.Error("获取进程对象失败", zap.Error(err), zap.Int("pid", mp.PID))
		return false
	}
	// 获取进程的磁盘 I/O 计数
	ioCounters, err := p.IOCountersWithContext(context.Background())
	if err != nil {
		d.logger.Error("获取进程磁盘 I/O 失败", zap.Error(err), zap.Int("pid", mp.PID))
		return false
	}

	if d.lastIOCounters == nil {
		d.lastIOCounters = ioCounters
		return false // 第一次检查，无法判断活动
	}

	// 如果读写字节数有变化，则认为活跃
	isActive := (ioCounters.ReadBytes > d.lastIOCounters.ReadBytes) || (ioCounters.WriteBytes > d.lastIOCounters.WriteBytes)
	d.lastIOCounters = ioCounters // 更新上一次的计数
	return isActive
}

func (d *diskIOActivityChecker) GetName() string {
	return "DiskIOActivityChecker"
}

// NewMemoryActivityChecker 创建一个实际的内存活动检查器。
func NewMemoryActivityChecker(logger *zap.Logger) ActivityChecker {
	return &memoryActivityChecker{logger: logger}
}

type memoryActivityChecker struct {
	logger *zap.Logger
	lastRSS uint64 // 记录上一次的 RSS
}

func (m *memoryActivityChecker) CheckActivity(mp *MonitoredProcess) bool {
	p, err := mp.Process()
	if err != nil {
		m.logger.Error("获取进程对象失败", zap.Error(err), zap.Int("pid", mp.PID))
		return false
	}
	// 获取进程的内存信息
	memInfo, err := p.MemoryInfoWithContext(context.Background())
	if err != nil {
		m.logger.Error("获取进程内存信息失败", zap.Error(err), zap.Int("pid", mp.PID))
		return false
	}
	...
	// 如果 RSS (Resident Set Size) 有显著变化，则认为活跃
	// 阈值可以根据实际情况调整，例如 1MB 的变化
	isActive := (memInfo.RSS > m.lastRSS && memInfo.RSS - m.lastRSS > 1024*1024) || (m.lastRSS > memInfo.RSS && m.lastRSS - memInfo.RSS > 1024*1024)
	m.lastRSS = memInfo.RSS // 更新上一次的 RSS
	return isActive
}

func (m *memoryActivityChecker) GetName() string {
