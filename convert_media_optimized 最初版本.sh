...
if (( BASH_VERSINFO[0] < 4 )); then
    printf "❌ \033[1;31m错误:\033[0m 此脚本需要 Bash 版本 4 或更高。\n"
...
# UI & UX 终极稳定版本 - 专注UI稳定性
VERSION="8.7.0-FIXED-LOGIC"
LOG_DIR=""
CONVERSION_LOG=""
REPORT_FILE=""
MODE=""
TARGET_DIR=""
# 自动计算并发数，75% CPU 核心数，最低为 4
CONCURRENT_JOBS=$(sysctl -n hw.ncpu 2>/dev/null | awk '{print int($1*0.75)}' || echo "4")
if (( CONCURRENT_JOBS == 0 )); then CONCURRENT_JOBS=4; fi # 确保并发数至少为4
ENABLE_HW_ACCEL=1
TEMP_DIR=""
RESULTS_DIR=""
ALL_FILES_COUNT=0
# 声明变量，后续由 aggregate_results 填充
SUCCESS_COUNT=0
FAIL_COUNT=0
SKIP_COUNT=0
SIZE_REDUCED=0
...
SIZE_UNCHANGED=0
TOTAL_SAVED=0
TOTAL_SIZE_INCREASED_SUM=0
SMART_DECISIONS_COUNT=0
LOSSLESS_WINS_COUNT=0
QUALITY_ANALYSIS_COUNT=0
declare -a FAILED_FILES=()
declare -a QUALITY_STATS=()
declare -a LOG_BUFFER=()

# UI Formatting Variables (using 256 colors for richer palette)
BOLD='\033[1m'
DIM='\033[2m'
RESET='\033[0m'
CLEAR_LINE="\r\033[K"
...
# Dynamic Colors
COLOR_BLUE='\033[38;5;39m'     # 深海蓝
COLOR_CYAN='\033[38;5;45m'     # 活力青
COLOR_GREEN='\033[38;5;47m'    # 翠绿色
COLOR_YELLOW='\033[38;5;220m'  # 鲜艳黄
COLOR_ORANGE='\033[38;5;202m'  # 炽热橙
COLOR_RED='\033[38;5;196m'     # 醒目红
COLOR_GRAY='\033[38;5;242m'    # 温暖灰
COLOR_LIGHT_GRAY='\033[38;5;250m' # 浅灰
COLOR_VIOLET='\033[38;5;129m'  # 梦幻紫
...
COLOR_HIGHLIGHT=$COLOR_VIOLET
COLOR_STATS=$COLOR_ORANGE
COLOR_SUBTLE=$COLOR_GRAY
...
init_logging() {
    ...
    LOG_DIR="$TARGET_DIR"
    # [FIX] 日志文件增加模式，更精细化
    CONVERSION_LOG="$LOG_DIR/conversion_log_${MODE}_${timestamp}.txt"
    REPORT_FILE="$LOG_DIR/conversion_report.txt"
...
is_animated() {
    local mime
    mime=$(get_mime_type "$1")
    case "$mime" in
        image/gif|image/webp|image/avif)
            local frames
            frames=$(ffprobe -v quiet -select_streams v:0 -show_entries stream=nb_frames -of csv=p=0 "$1" 2>/dev/null || echo "1")
...
is_live_photo() {
    local basename dir mov_file
    basename=$(basename "$1")
    dir=$(dirname "$1")
    if [[ "$basename" =~ ^IMG_[0-9]+\.HEIC$ ]]; then
        mov_file="${dir}/${basename%.HEIC}.MOV"
        [[ -f "$mov_file" ]]
    else
        return 1
    fi
...
# [FIX] 采用方案文档中的 get_adaptive_threshold 函数
get_adaptive_threshold() {
    local mime="$1" size="$2"
    case "$mime" in
        image/gif)
            if [[ $size -gt 2097152 ]]; then echo "20"; else echo "35"; fi ;;
...
generate_first_lossy_image() {
    ...
    if is_animated "$input"; then
        ...
        if magick "$input" -quality "$quality" "$output" >/dev/null 2>>"$CONVERSION_LOG"; then
            return 0
        fi
    fi
...
# [FIX] 采用方案文档中的 make_smart_decision 逻辑
make_smart_decision() {
    ...
    if [[ $lossless_size -lt $((orig_size / 5)) && $lossless_size -lt $((lossy_size / 2)) ]]; then
        echo "USE_LOSSLESS_EXTREME"
        return
    fi
    local gap=0
    if [[ $orig_size -gt 0 ]]; then
        gap=$(( (lossy_size - lossless_size) * 100 / orig_size ))
    fi
...
unified_smart_analysis_image() {
    ...
    local complexity=$(estimate_complexity "$input")
    local lossless_file="${temp_output_base}_lossless.avif"
    local first_lossy_file="${temp_output_base}_first.avif"
    ...
    # [FIX] 在函数内就为 decision_tag 赋值
    local decision_tag=""
    if [[ "$MODE" == "quality" ]]; then
        if [[ $lossless_size -gt 0 && $lossless_size -lt $lossy_size ]]; then
            decision_tag="QUALITY_LOSSLESS_OPTIMAL"
        else
            decision_tag="QUALITY_LOSSLESS_FORCED"
        fi
...
continue_static_exploration() {
    local input="$1" temp_output_base="$2" original_size="$3"
    local quality_levels=(70 55 40); local best_file=""; local best_size=$original_size; local best_quality=""
...
continue_animated_exploration() {
    local input="$1" temp_output_base="$2" original_size="$3"
    local crf_levels=(40 50); local best_file=""; local best_size=$original_size; local best_crf=""
...
# [FIX] 全面重构视频效率模式，引入无损HEVC对比
convert_video_efficiency_mode() {
    local input="$1" temp_output_base="$2" original_size="$3"
    local lossless_file="${temp_output_base}_lossless.mov"
    local first_lossy_file="${temp_output_base}_first.mp4"
    
    # 并行生成无损 HEVC 和初次有损 AV1 (CRF 32)
...
process_file() {
    ...
    # [FIX] 统一返回格式为 result|quality_stat|temp_file|decision_tag
    ...
    if [[ -n "$result" ]]; then
        ...
        if [[ $new_size -gt 0 ]]; then
            ...
            if [[ "$MODE" == "quality" || $new_size -lt $original_size ]]; then should_replace=1; fi
            if [[ $should_replace -eq 1 ]]; then
                ...
                # [FIX] 将 decision_tag 写入结果文件
                echo "SUCCESS|$original_size|$new_size|$quality_stat|$decision_tag" > "$result_file"
            else
                log_message "WARN" "转换后文件增大，不替换: $basename ($(numfmt --to=iec "$original_size") -> $(numfmt --to=iec "$new_size"))"
                echo "SKIP|$basename" > "$result_file"; rm -f "$temp_file" 2>/dev/null
            fi
        else
...
# [FIX] 重构 aggregate_results 以解析新的结果文件格式并正确计数
aggregate_results() {
...
generate_report() {
    ...
    echo -e "${BOLD}${COLOR_CYAN}--- 大小变化 ---${RESET}"
    echo -e "  📉 体积减小文件: ${SIZE_REDUCED}"
    echo -e "  ${BOLD}💰 最终空间节省: ${COLOR_SUCCESS}${saved_space_str}${RESET}"
    echo
...
${BOLD}${COLOR_CYAN}核心特性:${RESET}
  • ${COLOR_SUCCESS}恢复稳定转换内核${RESET} - 基于验证版本的核心算法
...
execute_conversion_task() {
    ...
    if [[ $ALL_FILES_COUNT -gt 0 ]]; then
        ...
        if [[ $effective_total -gt 0 ]]; then
            ...
            success_rate="100.0" # 如果所有文件都被跳过，也算100%成功
        fi
    fi
...
interactive_session_loop() {
    while true; do
        ...
        if [[ "$confirm_choice" == "n" ]]; then
            echo -e "${COLOR_YELLOW}ℹ️ 操作已取消，返回主菜单。${RESET}"
            continue
        fi
        ...
        if [[ "$continue_choice" == "n" ]]; then
            echo -e "${COLOR_SUCCESS}感谢使用媒体批量转换脚本！👋${RESET}"
            break
        fi
