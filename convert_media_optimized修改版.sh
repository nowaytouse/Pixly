...
VERSION="8.9.0-JXL-MOV-OPTIMIZED"
...
MAX_TEMP_FILES=1000 MAX_BACKUP_FILES=100
...
cleanup() {
    ...
    rm -f /tmp/conv_* /tmp/fixed_* /tmp/test_* /tmp/*.jxl /tmp/*.mov 2>/dev/null || true
...
create_safe_temp_dir() {
    ...
    local temp_dir="$base_dir/conv_$$_$(date +%s)"
...
cleanup_old_files() {
    ...
    [[ ! -d "$dir" ]] && return
    local file_count=$(find "$dir" -name "$pattern" -type f 2>/dev/null | wc -l | tr -d ' ')
...
generate_lossless_image() {
    ...
    if is_animated "$input"; then
        ...
        # 数学级无损JXL转换，使用极致参数
        if ! magick "$input" -quality 100 -define jxl:effort=9 -define jxl:distance=0 "$output" >/dev/null 2>>"$CONVERSION_LOG"; then
            log_message "ERROR" "无损静态JXL转换失败: $(basename "$input")"
            return 1
        fi
    fi; return 0
}
...
unified_smart_analysis_image() {
    ...
    local lossless_file="${temp_output_base}_lossless.jxl" first_lossy_file="${temp_output_base}_first.avif"
    ...
    if [[ "$MODE" == "quality" ]]; then
        ...
        if [[ -f "$lossless_file" && $lossless_size -gt 0 ]]; then
            local quality_type="JXL-Quality"; [[ "$decision_tag" == "QUALITY_LOSSLESS_OPTIMAL" ]] && quality_type+="-Optimal"
        ...
        case "$decision" in
            "USE_LOSSLESS_EXTREME"|"USE_LOSSLESS_BETTER"|"USE_LOSSLESS_SIGNIFICANT")
                rm -f "$first_lossy_file" 2>/dev/null
                if [[ -f "$lossless_file" && $lossless_size -gt 0 ]]; then
                    echo "JXL-Lossless|${lossless_file}|SMART_LOSSLESS"; return 0
                fi;;
...
convert_video_efficiency_mode() {
    local input="$1" temp_output_base="$2" original_size="$3"
    local lossless_file="${temp_output_base}_lossless.mov" first_lossy_file="${temp_output_base}_first.mov"
    ...
    case "$decision" in
        ...
        "USE_LOSSY_ACCEPTABLE")
            rm -f "$lossless_file" 2>/dev/null
            if [[ -f "$first_lossy_file" && $lossy_size -gt 0 ]]; then
                echo "AV1-CRF32-MOV|${first_lossy_file}|SMART_LOSSY"; return 0
            fi;;
        "EXPLORE_FURTHER")
            rm -f "$lossless_file" "$first_lossy_file" 2>/dev/null
            local second_lossy_file="${temp_output_base}_second.mov"
            ...
            if ffmpeg_quiet -y -i "$further_input_file" -c:v libsvtav1 -crf 45 -preset 7 -g 240 -c:a copy -avoid_negative_ts make_zero "$second_lossy_file" 2>>"$CONVERSION_LOG"; then
                ...
                echo "AV1-CRF45-MOV|${second_lossy_file}|SMART_LOSSY_EXPLORED"; return 0
            fi
            [[ "$further_input_file" != "$input" ]] && rm -f "$further_input_file";;
    esac
    rm -f "$lossless_file" "$first_lossy_file" 2>/dev/null; return 1
...
should_skip_file() {
    ...
    case "$mime" in
        image/*) target_ext=$([[ "$MODE" == "quality" ]] && echo "jxl" || echo "avif");;
        video/*) target_ext="mov";;
...
process_file() {
    ...
    # 创建安全的临时目录
    local safe_temp_dir=$(create_safe_temp_dir) || {
        log_message "ERROR" "无法创建临时目录: $(basename "$file")"
        echo "FAIL|$(basename "$file")" > "$result_file"
        return 1
    }
    local temp_output_base="$safe_temp_dir/conv_$(basename "$file" | tr ' ' '_')"
    ...
    if [[ "$MODE" == "efficiency" ]]; then
        ...
        backup_path=$(create_backup "$file" "$backup_dir")
        [[ -n "$backup_path" ]] && log_message "INFO" "创建备份: $(basename "$backup_path")"
    fi
...
main_conversion_loop() {
    ...
    export -f ffmpeg_quiet create_safe_temp_dir create_backup cleanup_old_files
    export MODE ENABLE_HW_ACCEL CONVERSION_LOG TEMP_DIR RESULTS_DIR MAX_TEMP_FILES MAX_BACKUP_FILES
...
show_help() {
    cat << EOF
${BOLD}${COLOR_BLUE}🚀 媒体批量转换脚本 v$VERSION (JXL+MOV优化版)${RESET}
${DIM}${COLOR_SUBTLE}（无损模式JXL+效率模式AVIF+视频MOV格式优化）${RESET}
用法: $0 [选项] <目录路径>
...
${BOLD}${COLOR_CYAN}版本更新:${RESET}
  ✨ 无损模式图片格式升级为JXL (更好的压缩比和质量)
  ✨ 效率模式视频格式统一为MOV (更好的兼容性)
  ✨ 效率模式无损尝试阶段也使用AVIF+MOV格式
  ✨ 优化了临时文件清理机制，提升脚本稳定性
...
interactive_mode() {
    ...
    echo -e "${DIM}${COLOR_SUBTLE}JXL+MOV优化版 - 专业、高效、稳定${RESET}"
    echo -e "====================================================\n"
...
check_dependencies() {
    ...
    if ! magick -list format 2>/dev/null | grep -q "JPEG XL"; then
        echo -e "${COLOR_RED}❌ 错误: ImageMagick 未支持 JPEG XL (JXL) 格式。${RESET}"
        echo -e "${DIM}${COLOR_SUBTLE}无损模式需要JXL支持。请更新ImageMagick: brew install imagemagick${RESET}"
        exit 1
    fi
}
